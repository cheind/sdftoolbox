...
from surfacenets import plotting


def plot_frames():
    fig, ax = plotting.create_figure("ortho")

    ijk = np.stack(np.meshgrid(range(3), range(3), range(3), indexing="ij"), -1)
    colors = np.ones((3, 3, 3, 4))
    colors[:] = matplotlib.colors.to_rgba(
    ...
    ax.quiver(
        0, 0, 0, 0, 1, 0, length=1.0, arrow_length_ratio=0.1, color="green"
    )
    ax.text(0.1, 0.7, 0.0, "j", color="k")

    ax.quiver(
        0, 0, 0, 0, 0, 1, length=1.0, arrow_length_ratio=0.1, color="blue"
    )
...
def plot_edges():
    ...
    ax.quiver(
        1, 0, 0, 1, 0, 0, length=1.0, arrow_length_ratio=0.1, color="purple"
    )
    ax.text(1.5, -0.1, -0.1, "(1,0,0,0)", color="k")

    ax.quiver(
        0, 1, 1, 0, 0, 1, length=1.0, arrow_length_ratio=0.1, color="purple"
    )
    ax.text(0, 2.1, 1.5, "(0,1,1,2)", color="k")

    ax.quiver(
        2, 0, 2, 0, 1, 0, length=1.0, arrow_length_ratio=0.1, color="purple"
    )
